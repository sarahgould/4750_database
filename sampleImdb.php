...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  if(!empty($_POST['btnAction']) && $_POST['btnAction'] == "Add") {
    addMovie($_POST['title'], $_POST['year'], $_POST['gross'], $_POST['runtime'], $_POST['rating']);
    $list_of_movies = getAllMovies();
  }
    ...
    setcookie("movie_to_view", $movie_to_view['title']);
  ...
  if(!empty($_POST['btnAction']) && $_POST['btnAction'] == "Confirm Update") {
    updateMovie($_POST['title'], $_POST['year'], $_POST['gross'], $_POST['runtime'], $_POST['rating']);
    $list_of_movies = getAllMovies();
  }
  if(!empty($_POST['btnAction']) && $_POST['btnAction'] == "Clear Form") {
    $open_menu = True;
  }
}
...
<div id="content" class="collapse" style="margin-left: 20px; margin-right: 20px;">
    <form name="mainForm" action="sampleImdb.php" method="post">
        ...
        <input type="text" class="form-control" name="title" required 
            value="<?php if ($movie_to_update!=null) echo $movie_to_update['title']?>"
        />
        </div>
        <div class="mb-3 mx-3">
            ...
            <input style="width:150px;" type="number" class="form-control" name="year"
                value="<?php if ($movie_to_update!=null) echo $movie_to_update['year']?>"
                />
            </div> 
            <div style="display:inline-block;"> 
            <h5>Gross:</h5>
            ...
            <div style="display:inline-block;">
            <h5>Rating:</h5>
            <input type="number" class="form-control" name="rating" required min="0" max="10"
                value="<?php if ($movie_to_update!=null) echo $movie_to_update['rating'] ?>"
                />
            </div>
        </div>
        <br>
...
<hr/>
    ...
    <div class="card" style="margin-left: 25px; width: 600px; padding:5px;">
        <h5 style="color:black; font-family: Impact; font-size:30px"><?php echo $movie['title']; ?> </h5>
        Released Year: <?php echo $movie['year']; ?> <br>
        Gross: <?php echo $movie['gross']; ?> <br>
        Runtime (min): <?php echo $movie['runtime']; ?> <br>
        IMDB Rating: <?php echo $movie['rating']; ?> <br>
        <div>
            <div style="display:inline-block;">
